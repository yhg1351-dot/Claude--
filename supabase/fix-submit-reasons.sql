-- 제출 함수가 "토큰이 틀림"(invalid)과 "내용이 틀림"(bad_data: 사진 수 초과, 경로 불일치 등)을 구분해 돌려주도록 합니다.
-- 학생 앱이 내용 오류를 토큰 오류로 오해해 재접속·재전송을 반복하지 않게 하기 위한 것입니다.
-- Supabase SQL 편집기에서 이 파일 전체를 붙여 넣고 실행하세요. 데이터는 바뀌지 않으며 여러 번 실행해도 안전합니다.
